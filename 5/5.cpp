	...
	if (fin)
	{
		if (fscanf(fin, "%d", quantity_points) > 0)
		{
			...
			if (arr)
			{
				...
				*quantity_points = i;
				segment* temp = (segment*)realloc(arr, sizeof(segment) * (*quantity_points));
				if (temp)
				{
					arr = temp;
				}
...
int check_intersection_for_segments(segment AB, segment CD)
{
	...
	double z1 = (AB_AC * AB_AD);
	double z2 = (CD_CB * CD_CA);
	if (((z1 <= 0) && (z2 <= 0))) return 1;
	else return 0;
}
int check_intersection_for_broken_straight_line(segment* arr, int quantity_segments)
{
	if ((arr[0].x1 == arr[quantity_segments - 1].x2) && ((arr[0].y1 == arr[quantity_segments - 1].y2))) return 1;
	for (int i = 0; i < quantity_segments-1; i++)
	{
		for (int j = i + 2; j < quantity_segments; j++)
		{
